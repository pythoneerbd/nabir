...
def newsletter_subscribe(request):
    ...
    if form.is_valid():
        ...
        if NewsLetterUsers.objects.filter(email=instance.email).exists():
            ...
            html_template = get_template('/templates/newsletters/subscribe_email_message.html').render()
...
def newsletter_unsubscribe(request):
    ...
    if form.is_valid():
        ...
        if NewsLetterUsers.objects.filter(email=instance.email).exists():
            ...
            with open(settings.BASE_DIR + '/newsletters/unsubscribe_email_message.txt') as f:
                signup_message = f.read()
            message = EmailMultiAlternatives(subject=subject, body=signup_message,
                                             from_email=from_email, to=to_email)
